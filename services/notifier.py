    ...
    try:
        ...
        for row in rows:
            sub = Subscription(
                ...
                service=row["service"],
                label=row.get("label"),
                sum_min=row.get("sum_min"),
                sum_max=row.get("sum_max"),
                rating_min=row.get("rating_min"),
                period_min=row.get("period_min"),
                period_max=row.get("period_max"),
                interest_min=row.get("interest_min"),
                require_employed=bool(row["require_employed"]) if row.get("require_employed") is not None else None,
                min_settled_loans=row.get("min_settled_loans") if row.get("min_settled_loans") else None,
            )
